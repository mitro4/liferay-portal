...
<c:if test="<%= !skipEditorLoading %>">
	<liferay-util:html-top outputKey="js_editor_ckeditor_skip_editor_loading">
		...
		<script src="<%= HtmlUtil.escape(PortalUtil.getStaticResourceURL(request, themeDisplay.getCDNHost() + themeDisplay.getPathJavaScript() + "/editor/ckeditor45/ckeditor.js", javaScriptLastModified)) %>" type="text/javascript"></script>

		<c:if test="<%= inlineEdit && Validator.isNotNull(inlineEditSaveURL) %>">
			<script src="<%= HtmlUtil.escape(PortalUtil.getStaticResourceURL(request, themeDisplay.getCDNHost() + themeDisplay.getPathJavaScript() + "/editor/ckeditor45/main.js", javaScriptLastModified)) %>" type="text/javascript"></script>
		</c:if>
...
<aui:script use="<%= modules %>">
	...
	var createEditor = function() {
		...
		String fileBrowserUrl = PortalUtil.getPathContext() + 
								"/html/js/editor/ckeditor45/editor/filemanager/browser/liferay/browser.html?Connector=" + 
								connectorURL + fileBrowserParams;	
		
		%>
		
		window['<%=name %>FileBrowserUrl']='<%= fileBrowserUrl %>';
		
		window.localStorage.setItem('LCKE_customConfig', '<%= PortalUtil.getPathContext() %>/html/js/editor/ckeditor45/<%= HtmlUtil.escapeJS(ckEditorConfigFileName) %>?p_p_id=<%= HtmlUtil.escapeJS(portletId) %>&p_main_path=<%= HtmlUtil.escapeJS(mainPath) %>&colorSchemeCssClass=<%= HtmlUtil.escapeJS(themeDisplay.getColorScheme().getCssClass()) %>&contentsLanguageId=<%= HtmlUtil.escapeJS(contentsLanguageId) %>&cssClasses=<%= HtmlUtil.escapeJS(cssClasses) %>&cssPath=<%= HtmlUtil.escapeJS(themeDisplay.getPathThemeCss()) %>&doAsGroupId=<%= HtmlUtil.escapeJS(String.valueOf(doAsGroupId)) %>&doAsUserId=<%= HtmlUtil.escapeJS(doAsUserId) %>&imagesPath=<%= HtmlUtil.escapeJS(themeDisplay.getPathThemeImages()) %>&inlineEdit=<%= inlineEdit %><%= configParams %>&languageId=<%= HtmlUtil.escapeJS(LocaleUtil.toLanguageId(locale)) %>&name=<%= name %>&resizable=<%= resizable %>');
		window.localStorage.setItem('LCKE_filebrowserBrowseUrl', '<%= PortalUtil.getPathContext() %>/html/js/editor/ckeditor45/editor/filemanager/browser/liferay/browser.html?Connector=<%= connectorURL %><%= fileBrowserParams %>');
		window.localStorage.setItem('LCKE_toolbar', '<%= TextFormatter.format(HtmlUtil.escapeJS(toolbarSet), TextFormatter.M) %>');
		...
		<c:choose>
			...
			'<%= name %>',
			{
				customConfig: '<%= PortalUtil.getPathContext() %>/html/js/editor/ckeditor45/<%= HtmlUtil.escapeJS(ckEditorConfigFileName) %>?p_p_id=<%= HtmlUtil.escapeJS(portletId) %>&p_main_path=<%= HtmlUtil.escapeJS(mainPath) %>&colorSchemeCssClass=<%= HtmlUtil.escapeJS(themeDisplay.getColorScheme().getCssClass()) %>&contentsLanguageId=<%= HtmlUtil.escapeJS(contentsLanguageId) %>&cssClasses=<%= HtmlUtil.escapeJS(cssClasses) %>&cssPath=<%= HtmlUtil.escapeJS(themeDisplay.getPathThemeCss()) %>&doAsGroupId=<%= HtmlUtil.escapeJS(String.valueOf(doAsGroupId)) %>&doAsUserId=<%= HtmlUtil.escapeJS(doAsUserId) %>&imagesPath=<%= HtmlUtil.escapeJS(themeDisplay.getPathThemeImages()) %>&inlineEdit=<%= inlineEdit %><%= configParams %>&languageId=<%= HtmlUtil.escapeJS(LocaleUtil.toLanguageId(locale)) %>&name=<%= name %>&resizable=<%= resizable %>',
				filebrowserBrowseUrl: '<%= PortalUtil.getPathContext() %>/html/js/editor/ckeditor45/editor/filemanager/browser/liferay/browser.html?Connector=<%= connectorURL %><%= fileBrowserParams %>',
