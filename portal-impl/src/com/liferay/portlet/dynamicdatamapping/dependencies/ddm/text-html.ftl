...
<@aui["field-wrapper"] data=data helpMessage=escape(fieldStructure.tip) label=escape(label) required=required>
	...
	<@liferay_ui["input-editor"] contentsLanguageId="${requestedLocale}" editorImpl="editor.wysiwyg.portal-web.docroot.html.portlet.journal.edit_article_content_xsd_el.jsp" initMethod="${namespacedFieldName}InitEditor" name="${namespacedFieldName}Editor" onBlurMethod="${namespacedFieldName}OnBlurEditor" skipEditorLoading=skipEditorLoading />
